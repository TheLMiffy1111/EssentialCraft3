	...
	public void renderLivingSpecials(RenderLivingEvent.Specials.Post<?> event) {
		if(event.getEntity() instanceof EntityPlayer) {
			...
			if(BaublesApi.getBaublesHandler(p) != null &&
					!BaublesApi.getBaublesHandler(p).getStackInSlot(3).isEmpty() &&
					...
					try {
						...
						for(int i = 0; i < gc.inventorySlots.inventorySlots.size(); ++i) {
							...
							if(slt.inventory instanceof TileEntity || slt.inventory instanceof InventoryBasic) {
								...
								Minecraft.getMinecraft().fontRenderer.drawString(""+slt.slotNumber, (k+slt.xPos)*2, (l+slt.yPos)*2, 0x000000);
								GlStateManager.popMatrix();
									...
									if(RenderHandlerEC.slotsTable.containsKey(inv)) {
										...
										if(accessibleSlots.containsKey(slt.slotNumber)) {
											...
											if(lst != null && !lst.isEmpty()) {
												...
												GlStateManager.pushMatrix();
												GlStateManager.scale(0.5F, 0.5F, 0.5F);
												Minecraft.getMinecraft().fontRenderer.drawString(""+d, (k+slt.xPos)*2, (l+slt.yPos+12)*2, 0x000000);
												GlStateManager.popMatrix();
											}
										}
									}
	...
	public void onPlayerTick(TickEvent.PlayerTickEvent event) {
		...
		if(event.player.getEntityWorld().isRemote && event.phase == Phase.START)
			if(event.player instanceof EntityPlayerSP) {
				...
				if(isNightVisionActive && (player.inventory.armorInventory.get(3).isEmpty() || !(player.inventory.armorInventory.get(3).getItem() instanceof ItemComputerArmor))) {
					isNightVisionActive = false;
					Minecraft.getMinecraft().player.removePotionEffect(MobEffects.NIGHT_VISION);
				}

				if(ClientProxy.kbArmorVision.isKeyDown() && !isNightVisionKeyDown && player.inventory.armorInventory.get(3)!=null && player.inventory.armorInventory.get(3).getItem() instanceof ItemComputerArmor) {
	...
	public void pre(RenderGameOverlayEvent.Pre event) {
		try {
			if(event.getType() == ElementType.CROSSHAIRS) {
				...
				if(!Minecraft.getMinecraft().player.inventory.armorInventory.get(3).isEmpty() && Minecraft.getMinecraft().player.inventory.armorInventory.get(3).getItem() instanceof ItemComputerArmor) {
					ScaledResolution res = new ScaledResolution(mc);
					float r = 10;
			...
			if(event.getType() == RenderGameOverlayEvent.ElementType.HEALTH) {
				if(Minecraft.getMinecraft().player.getActivePotionEffect(PotionRegistry.mruCorruption)!=null) {
					...
					if(b != null && b instanceof BlockWindRune) {
						...
						if(rune != null) {
							if(!p.getHeldItemMainhand().isEmpty()) {
								WindImbueRecipe rec = WindImbueRecipe.getRecipeByInput(p.getHeldItemMainhand());
								if(rec != null) {
